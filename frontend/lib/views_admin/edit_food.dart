...
class _EditFoodScreenState extends State<EditFoodScreen> {
  ...
  late final TextEditingController _detailCtr;
  ...
  void initState() {
    super.initState();
    // Initialize controllers with existing food item data
    _nameCtr = TextEditingController(text: widget.foodItem['name'] ?? '');
    _priceCtr = TextEditingController(
      text: widget.foodItem['price']?.toString() ?? '0',
    );
    _detailCtr = TextEditingController(
      text: widget.foodItem['description'] ?? '',
    );
    _existingImageUrl =
        widget.foodItem['image_url']?.isNotEmpty == true
            ? widget.foodItem['image_url']
            : null;
  }
  ...
  Future<String?> _uploadImageToS3() async {
    if (_selectedImage == null)
      return _existingImageUrl; // Retain existing image if no new image
    ...
    try {
      ...
      return _existingImageUrl; // Fallback to existing image on failure
    } finally {
  ...
  Future<void> _saveItem() async {
    ...
    try {
      ...
      if (_selectedImage != null && imgUrl == _existingImageUrl) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Không thể tải ảnh mới')));
        return;
      }
      ...
      final res = await http.put(
        Uri.parse(
          'https://api.df.nguyenquangcuong.pro/admin/restaurant/${widget.restaurantId}/food/${widget.foodItem['id']}',
        ),
        headers: {
          'Authorization': 'Bearer $tkn',
          'Content-Type': 'application/json',
      ...
      if (res.statusCode == 200) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Đã cập nhật món: ${_nameCtr.text}')),
        );
        Navigator.pop(context, true); // Return true to refresh the list
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Lỗi cập nhật món: ${res.statusCode}')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(
  ...
  void _reset() => setState(() {
    _nameCtr.text = widget.foodItem['name'] ?? '';
    _priceCtr.text = widget.foodItem['price']?.toString() ?? '0';
    _detailCtr.text = widget.foodItem['description'] ?? '';
    _selectedImage = null;
    _fileName = null;
    _fileExt = null;
    _existingImageUrl =
  ...
  Widget _Label(String txt) => Padding(
    ...
    child: Text(
      ...
      style: const TextStyle(
        ...
        fontSize: 12,
        letterSpacing: .6,
        color: _EditFoodScreenState._greyHint,
        fontWeight: FontWeight.w600,
      ),
    ),
  );
        ...
        TextButton(
          ...
          child: const Text(
            ...
            style: TextStyle(
              fontFamily: 'Roboto',
              fontSize: 13,
              color: _EditFoodScreenState._primary,
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: List.generate(3, (i) {
        if (i == 0 && (image != null || existingImageUrl != null)) {
          return SizedBox(
            width: box,
            height: box,
            child: Container(
...
class _RoundedInput extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: _EditFoodScreenState._fill,
        borderRadius: BorderRadius.circular(10),
        border: Border.all(color: _EditFoodScreenState._greyBorder),
      ),
      child: TextFormField(
        ...
        decoration: InputDecoration(
          ...
          hintStyle: const TextStyle(
            fontFamily: 'Roboto',
            color: _EditFoodScreenState._greyText,
          ),
        ),
